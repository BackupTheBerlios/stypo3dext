 ...
 * Modified by Vincent MAURY <vmaury(at)nerim.net
 */
...
//echo $row["title"]."<br>";
			...
			if ($depth>1 && $this->expandNext($newID) && !$row["php_tree_stop"])	{
				$nextCount=$this->getTree($newID, $depth-1, $this->makeHTML?$depthData.'<IMG src="'.$this->backPath.'t3lib/gfx/ol/'.$LN.'.gif">':'', $blankLineCode.",".$LN);
			...
			if ($this->makeHTML)	{	
				...
				$HTML.=$this->wrapStop($this->wrapIcon('<IMG src="typo3/gfx/i/pages.gif">',$row),$row);
			}
...
//		debug($dirs);
	...
	function PMicon($row,$a,$c,$nextCount,$exp)	{
		$PM = "join";
		$BTM = ($a==$c)?"bottom":"";
		$icon = '<IMG src="'.$this->backPath.'t3lib/gfx/ol/'.$PM.$BTM.'.gif">';
		return $icon;
	}
